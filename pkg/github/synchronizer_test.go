...
func TestSynchronizer_Sync(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			teams := make([]*api.GitHubTeam, len(tc.teamMemberLogins))
...
func teamWithUserLogins(arr []string, teamID, OrgID int64) *api.GitHubTeam {
	users := make([]*api.GitHubUser, len(arr))
	for i, s := range arr {
		users[i] = &api.GitHubUser{Login: s}
	}
	return &api.GitHubTeam{
		OrgId:  OrgID,
		TeamId: teamID,
		Users:  users,
	}
